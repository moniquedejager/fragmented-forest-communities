...
sel <- (df3$mu != 3)&(df3$f_hab_loss %in% c(0, 0.25, 0.5, 0.75, 0.95))&(df3$dispersal == 'different')
df3 <- df3[sel,]
df3$maxLambda <- 1
...
p3 <- ggplot(sdf, aes(x=f_loss*100, y=y, color=mu2)) + 
  ...
  geom_point(position=pd) + 
  scale_color_manual(values=cbp1, name='') + 
  facet_grid(cols=vars(dispNhood), rows=vars(ylab), scales='free_y', switch='y') + 
  geom_errorbar(aes(ymin=ymins, ymax=ymaxs), 
                width=0, position=pd) + 
  xlab('% Habitat loss') + 
  ylab('') + 
