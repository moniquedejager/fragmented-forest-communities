...
fragment_community_similar_dispersal <- function(n_ind, Pm_range,clustering, sim_nr, mutation_rate, max_mutation){
  library(ggplot2)
  source('src 04-2024 no metappn/simulate_community_dynamics.R')
  source('./src 04-2024 no metappn/write_data_to_files_similar_dispersal.R')
  ...
  for (iFrag in 1:20){
    ...
    if (!file.exists(filename)){
     ...
     if ((frag > 0)&(startup == TRUE)){
      ...
      rv$frag     <- frag
      rv$sim_nr   <- sim_nr
      write_data_to_files_similar_dispersal(rv)
    }
  }
}
...
result_parallel <- future.apply::future_lapply(1:length(dat$n_ind), function(i) {
  fragment_community_similar_dispersal(dat$n_ind[i], dat$Pm_range[i], 
